...
class Config
{
	...
	public function __construct()
	{
		...
		\add_action( 'admin_enqueue_scripts', array( $this, 'adminEnqueue' ), 99 );
		\add_action( 'wp_ajax_tracking_code_installer', array( $this, 'processAjaxRequest' ), 99 );
	}
			...
			if( !empty( $obj_data ) ){
				...
				if( empty( $obj_data['is_delete'] ) ){
					...
					if( wp_verify_nonce( $data['tci-security-token'], $this->nonceAction ) ){
						...
						$saved_data = get_option( 'tc_installer_data', array() );
	...
	*/
	...
	{
		$opt = get_option( 'tc_installer_data', array() );
		$output = '';
