		...
		$output .= '<ul class="tc_installer-required-plugins-list">';
			foreach ($this->data as $plugin_slug => $plugin) {
				...
				if( !empty( $plugin['plugin_uri'] ) ){
					$button = '<a href="'. esc_url_raw( $plugin['plugin_uri'] ) .'" class="tc_installer-plugin-info-button" target="_blank">'. __( 'Get the plugin', 'tracking-code-installer' ) .'</a>';
				}
				else{
					$button = '<a href="#" onclick="return false;" class="tc_installer-plugin-info-button disabled">'. __( 'Get the plugin', 'tracking-code-installer' ) .'</a>';
				}
